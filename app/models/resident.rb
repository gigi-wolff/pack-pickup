class Resident < ActiveRecord::Base
  ...
  #validates_format_of :personal_phone, :with => /\A^[0-9]\z/, message:"should contain only digits please"
  validates :personal_phone, length: {is: 10, message: "is the wrong length (should be 10 digits)"}
    #:numericality => {:only_integer => true, message: "numbers only: 1234567890"}, 

  #validates_format_of :work_phone, :with => /\A^[0-9]\z/, message:"should contain only digits please"
  validates :work_phone, length: {is: 10, message: "is the wrong length (should be 10 digits)"}
